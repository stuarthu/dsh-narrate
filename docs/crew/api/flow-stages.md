...
3. `voice` 写进文件之前，**必须**把音频开头和结尾的静音裁到不超过 0.15 秒。理由：不裁的话，画面和声音的起点会差开，而且每一句都差一点，越往后越歪。
4. `audioPath` 指向的文件必须真的存在，且必须是 `voice` 已经裁过静音的那一个。
...
| 侧 | 测试证明什么 |
| --- | --- |
| `voice` 侧 | 喂一段已知长度 2.000 秒、前后各带 1 秒静音的音频。测试证明：写回的 `durationSec` 是 2.000 ± 0.01，且 `audioPath` 指的文件前后静音都不超过 0.15 秒 |
