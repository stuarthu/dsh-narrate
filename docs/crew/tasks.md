...
7. **`T-06` 的文字层必须按标签的稀有度加权。** 真实素材证明了这一点：从 archive.org 下载的素材每个带 20 到 32 个标签，大半是 `HD`、`1920x1080`、`H.264`、`Fair Use` 这类**每个素材都有**的格式噪音，真正有用的 `forest`、`mountain` 被淹在里面。一个所有素材都有的标签携带零信息量。不加权的话第二层漏斗基本白做。
