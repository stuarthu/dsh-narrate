...
| `T-09` | M6 | 逐句配音循环、把逐句音频拼成一条纯音频给停点 4、分批拼接（单次上限 20 段）、横屏和竖屏两套字幕规则（竖屏字号更大、每行更少字、位置更高）、以及 `Q-6` 的补长规则（缺口 ≤ 20% 放慢，> 20% 循环）。**补长规则要改 `src/render/segment.js`**——那是 `T-01` 的文件，但补长必须在裁那一段画面的那一刻生效，而 `T-01` 跑的时候 `Q-6` 还没定。这是第三处写明理由的例外 | `src/voice/speak.js`<br>`src/render/concat.js`<br>`src/render/subtitle.js`<br>`src/render/segment.js`（只加补长规则）<br>`test/render-concat.test.js`<br>`test/subtitle.test.js` | `T-01` | `flow-stages.md` | `A-8` `A-9` `A-16` `A-17` `A-18` |
...
5. **字幕必须在缩放到目标分辨率之后再烧，不能烧在素材原始分辨率上。** `FontSize` 和 `MarginV` 是绝对像素：一段 640x360 的素材烧 42px 字幕，字会占满画面；之后再缩放到 1080p 又会模糊。所以拼接那一步的顺序必须是**先缩放、再烧字幕**。实测发现的：竖屏的 `MarginV=220` 在一个 360 高的素材上会把字幕推到画面上半部分。
...
10. **中文文件名不分词。** `服务器机柜-特写` 能切出三个词，`服务器机柜特写` 只算一个。`T-03` 的文件名读取器只能做到这个程度，不要指望它。
