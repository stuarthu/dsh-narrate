...
9. **稀有度权重永远不到 0，这是故意的。** 原来的 `log((N+1)/(df+1))` 在"所有素材都有这个词"时正好等于 0，于是**只放了一段素材的文件夹里每个词都是 0 分，每一句都算没配上**，插件看起来完全不工作。用户先放一两段试试是很常见的。改成 `log((N+1)/(df+0.5))`：36 段素材时样板词 0.04、只有一段有的词 3.2，倍数关系没变，但不再是 0。原来记的"`fair use` 权重 0.000"这个数字也随之作废。
10. **跨语言匹配只能靠 agent 给的英文查询。** 中文文稿对英文素材元数据，字符二元组的交集是空的——36 段真实素材上实测每一段都是 0 分。所以英文查询在实践中几乎是必需的，不只是第 3 层的可选项。已写进契约版本 11。原来这一条写的是"文字层必须按稀有度加权"，那一条已经做完了：`fair use` 权重 0.000，`fire` 2.225。 真实素材证明了这一点：从 archive.org 下载的素材每个带 20 到 32 个标签，大半是 `HD`、`1920x1080`、`H.264`、`Fair Use` 这类**每个素材都有**的格式噪音，真正有用的 `forest`、`mountain` 被淹在里面。一个所有素材都有的标签携带零信息量。不加权的话第二层漏斗基本白做。
14. **下载来源的标题常带格式尾巴**（`Mountain Fog (Free to Use HD Stock Video Footage)`）。去掉要靠猜规律，所以不做。模型读到它不会理解错，只是稍微啰嗦。
