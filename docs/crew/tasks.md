...
4. **PRD 的 `Q-6` 没定**（素材不够长时循环、定格还是放慢），会卡住 `T-09` 的一部分。`T-01` 不受影响，因为骨架用一段足够长的素材。
5. **PRD 的 `Q-5` 现在只卡第 3 层漏斗。** 画面搜索需要那 2 GB 的向量层。不装也能跑，只是挑素材靠文字，质量差一些。这已经不再是一个阻塞问题，而是一个质量选项——契约版本 4 把它设计成了可退回的。
6. **`A-11` 里"画面上的字确实是那几个字"没法完全自动测。** 自动测试能证明画的是真字形不是方块（笔画多的字墨量明显多于笔画少的字），也能证明字幕文件内容和文稿一致。但"渲染出来的像素就是这几个字"要人眼看。这一条靠里程碑评审兜。
7. **默认语音引擎会联网，把旁白文字发给微软。** 已在 `README.md` 和 `README-zh.md` 的隐私一节披露，`T-10` 更新 README 时不许删掉那一节。
8. **跨语言匹配只能靠 agent 给的英文查询。** 中文文稿对英文素材元数据，字符二元组的交集是空的——36 段真实素材上实测每一段都是 0 分。所以英文查询在实践中几乎是必需的，不只是第 3 层的可选项。已写进契约版本 11。原来这一条写的是"文字层必须按稀有度加权"，那一条已经做完了：`fair use` 权重 0.000，`fire` 2.225。 真实素材证明了这一点：从 archive.org 下载的素材每个带 20 到 32 个标签，大半是 `HD`、`1920x1080`、`H.264`、`Fair Use` 这类**每个素材都有**的格式噪音，真正有用的 `forest`、`mountain` 被淹在里面。一个所有素材都有的标签携带零信息量。不加权的话第二层漏斗基本白做。
8. **下载来源的标题常带格式尾巴**（`Mountain Fog (Free to Use HD Stock Video Footage)`）。去掉要靠猜规律，所以不做。模型读到它不会理解错，只是稍微啰嗦。
9. **中文文件名不分词。** `服务器机柜-特写` 能切出三个词，`服务器机柜特写` 只算一个。`T-03` 的文件名读取器只能做到这个程度，不要指望它。
