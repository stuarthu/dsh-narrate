...
M6:  T-08 ✅ 、 T-09 ✅（两个可以同时跑，文件不重叠） → T-12 ✅ → T-10 ✅
     ← 四个任务都完成并评审过，但**里程碑本身还没验收**：`prd.md` 给 M6 定的
       验收方法是"在 dsh 里真做一条视频"，而 DeepSeek 账户余额不足，真会话跑不起来。
       见 review-m6.md 的"没做的事"。
```
...
9. **M6 评审时发现两条验收检查「数据算出来了但没人看得到」。** `A-26`（用过的英文查询）和 `A-27`（一段素材用了几次）——`assignShots` 早就算出来了，但 `narrate_shotplan` 的返回值里没有，agent 和用户都看不见。算出来不等于交付了。以后加验收检查时要问一句：**这个信息最终从哪个出口出去？**
10. **稀有度权重永远不到 0，这是故意的。** 原来的 `log((N+1)/(df+1))` 在"所有素材都有这个词"时正好等于 0，于是**只放了一段素材的文件夹里每个词都是 0 分，每一句都算没配上**，插件看起来完全不工作。用户先放一两段试试是很常见的。改成 `log((N+1)/(df+0.5))`：36 段素材时样板词 0.04、只有一段有的词 3.2，倍数关系没变，但不再是 0。原来记的"`fair use` 权重 0.000"这个数字也随之作废。
11. **跨语言匹配只能靠 agent 给的英文查询。** 中文文稿对英文素材元数据，字符二元组的交集是空的——36 段真实素材上实测每一段都是 0 分。所以英文查询在实践中几乎是必需的，不只是第 3 层的可选项。已写进契约版本 11。原来这一条写的是"文字层必须按稀有度加权"，那一条已经做完了：`fair use` 权重 0.000，`fire` 2.225。 真实素材证明了这一点：从 archive.org 下载的素材每个带 20 到 32 个标签，大半是 `HD`、`1920x1080`、`H.264`、`Fair Use` 这类**每个素材都有**的格式噪音，真正有用的 `forest`、`mountain` 被淹在里面。一个所有素材都有的标签携带零信息量。不加权的话第二层漏斗基本白做。
12. **下载来源的标题常带格式尾巴**（`Mountain Fog (Free to Use HD Stock Video Footage)`）。去掉要靠猜规律，所以不做。模型读到它不会理解错，只是稍微啰嗦。
13. **横屏素材做竖屏成片时，上下会有很大的黑边。** 加黑边保住了比例、不拉伸，这是诚实的做法，但抖音和 Shorts 上常见的做法是裁到满屏或者背景虚化填充。这是一个产品选择，不在 `T-09` 范围内，也没有测试假装它已经解决了。要改就是一个新的变更请求。
14. **拼接的验收要"完整解码一遍"，长片子会慢。** 一条十分钟的 1080p 成片解码一遍大概几十秒。值得：拷流拼坏的时候 ffmpeg **退出码是 0**，只量时长看不出来（实测一次只差 0.02 秒，但帧数少了四分之一）。宁可慢，不能交坏片子。
15. **中文文件名不分词。** `服务器机柜-特写` 能切出三个词，`服务器机柜特写` 只算一个。`T-03` 的文件名读取器只能做到这个程度，不要指望它。
