...
3. **`T-07` 需要一个"这算不算真匹配"的门槛，不能只看分数大于 0。** 真实素材验证时，一句"接下来讲编译器怎么做优化"在 36 段风景和纪录片素材里拿到了 3.48 分——没有一段相关，但分数不是 0。所以判断"缺素材"要靠相对门槛（比如低于这一句最高分的某个比例，或者低于全篇中位数），不能用绝对零。这一条直接决定缺素材报告准不准。
