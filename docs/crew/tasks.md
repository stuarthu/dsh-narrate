...
7. **`sources` 的出处是整节级别的，不是逐字段的。** 如果描述来自对话、而你手改了一个标签，出处可能把两者都记成 `chat:<日期>`。要逐字段精确，得在契约里加逐字段出处，现在不值得。已写在 `from-you.js` 的注释里。
8. **中文文件名不分词。** `服务器机柜-特写` 能切出三个词，`服务器机柜特写` 只算一个。`T-03` 的文件名读取器只能做到这个程度，不要指望它。
